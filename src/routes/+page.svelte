<script lang="ts">
	...
	import { warmCitywideFromMarkers } from '$lib/services/vivacity-counter/vivacity-citywide-load.js';
	import { warmEcoCounterFromPageData } from '$lib/services/eco-counter/eco-counter-warm.js';
	...
	$effect(() => {
		if (!browser) return;
		warmCitywideFromMarkers(data?.vivacityMarkers);
		warmEcoCounterFromPageData(data);
	});
	...
	function trackDataSourceView(dataSource: string) {
		...
		function getCountryFromTimezone() {
			try {
				...
				return timezoneMap[timezone] || timezone.split('/')[0];
			} catch (e) {
				return 'Unknown';
			}
		}
	...
	function handleProviderInitialized() {
		...
		// Track initial data source once all providers are loaded
		if (providersInitialized === totalProviders) {
			trackDataSourceView(selectedDataSource);
		}
	}

	function handleDataSourceChange() {
		
		// Track the data source change as a page view
		trackDataSourceView(selectedDataSource);
