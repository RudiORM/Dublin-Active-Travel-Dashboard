<script>
	...
	let {
		...
		chartType = 'hourly' // 'hourly' or 'daily' (changed from 'monthly')
	} = $props();
	...
	const timeSeriesData = $derived.by(() => {
		return vivacityCounterProvider.selectedLocationTimeSeriesData;
	});

	// Get color for the travel mode
	const seriesColor = $derived.by(() => {
		return getVivacityCounterColor(travelMode);
	});
	...
	const hourlyChartData = $derived.by(() => {
		console.log('VivacityCounterTimeSeries - processing hourly data for:', travelMode);
	...
	// Transform daily data for SingleItemTimeSeries (changed from monthly)
	const dailyChartData = $derived.by(() => {
		console.log('VivacityCounterTimeSeries - processing daily data for:', travelMode);
		
		if (!timeSeriesData || !timeSeriesData.dailyData || !timeSeriesData.dailyData[travelMode]) {
			console.log('No daily data available for travel mode:', travelMode);
			return [];
		}
		
		const dailyData = timeSeriesData.dailyData[travelMode];
	...
	const chartData = $derived.by(() => {
		return chartType === 'daily' ? dailyChartData : hourlyChartData;
	});
...
<div class="stats-section">
	...
	<div class="chart-section">
		...
		<SingleItemTimeSeries
			...
			date={chartType === 'daily' ? true : false}
		/>
