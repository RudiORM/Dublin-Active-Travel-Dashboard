...
import { getParkingStats } from '$lib/services/nta/nta-processor.js';
	...
	let explanationButton: HTMLButtonElement;

	// Grey color palette for parking stand types
	const PARKING_COLORS = {
		'Sheffield Stand': '#6B7280',      // Medium grey
		'Hoop Stand': '#9CA3AF',           // Light grey  
		'Wheel Rack': '#4B5563',           // Dark grey
		'Post and Rail': '#D1D5DB',        // Very light grey
		'Other': '#374151',                // Darker grey
		'Unknown': '#E5E7EB'               // Lightest grey
	};

	// Create chart data for StackedBarH component
	const chartData = $derived.by(() => {
		const parkingStats = getParkingStats(parkingData);
		if (!parkingStats) return [];
		
		const standTypes = (parkingStats as any).standTypes || {};
		return Object.entries(standTypes)
			.sort(([,a], [,b]) => (b as number) - (a as number))
			.map(([type, count]) => ({
				label: type,
				value: count as number,
				color: PARKING_COLORS[type as keyof typeof PARKING_COLORS] || PARKING_COLORS['Unknown']
			}));
	});
</script>
