<script>
	...
	let {
		...
		chartType = 'hourly', // 'hourly' | 'weekly' | 'monthly'
	...
	const weeklyChartData = $derived.by(() => {
		if (!timeSeriesData || !timeSeriesData.weeklyData || !timeSeriesData.weeklyData[travelMode]) {
			return [];
		}
...
<div class="stats-section">
	...
	<div class="stats-section-header">
		{#if titleClickable}
			<button type="button" class="title title-button" onclick={() => onTitleClick?.()}>
	...
	<div class="chart-section">
		{#if hasDisplayableChartData}
			<SingleItemTimeSeries
				...
				date={chartType === 'monthly' || chartType === 'weekly'}
			/>
		{/if}
	</div>
</div>
