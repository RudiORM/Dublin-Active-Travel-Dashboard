<script>
	...
	const weeklyChartData = $derived.by(() => {
		const series = timeSeriesData?.weeklyBars?.[travelMode];
	...
	const monthlyChartData = $derived.by(() => {
		const series = timeSeriesData?.monthlyBars?.[travelMode];
		if (!Array.isArray(series)) return [];
		return series.map((item) => ({
			date: item.date,
			value: Math.round(item.value * 100) / 100
	...
	const chartData = $derived.by(() => {
		if (chartType === 'weekly') return weeklyChartData;
		if (chartType === 'monthly') return monthlyChartData;
...
<div class="stats-section">
	...
	<div class="stats-section-header">
		{#if titleClickable}
			...
			>
				{#if underlineWord && title.includes(underlineWord)}
					{title.split(underlineWord)[0]}<span class="underlined-word">{underlineWord}</span>{title.split(underlineWord)[1] || ''}
				{:else}
					{title}
				{/if}
			</button>
